...
class App extends Component {
  ...
  isMatchPlaying = () => {
    this.intervalId = setInterval(this.decreaseSeconds, 1000)
  }
  ...
  onClickThumbnail = id => {
    ...
    if (matchImageUrl.id === id) {
      this.setState(prevState => ({
        score: prevState.score + 1,
        matchImageUrl: this.getMatchImage(),
      }))
    } else {
  ...
  updateActiveTabId = tabId => {
    this.setState({activeTabId: tabId, score: 0, seconds: 60})
  }

  render() {
    const {matchImageUrl, score, isPlaying, seconds} = this.state
    const filteredImagesList = this.getFilteredProjects()
    return (
        ...
        {isPlaying ? (
          <div className="container">
            <div>
              <img
                alt="match"
                className="match-big-image"
                src={matchImageUrl.imageUrl}
              />
            </div>
            ...
            <ul className="project-item-container">
              {filteredImagesList.map(eachImageItem => (
                <ImageItems
                  imageItem={eachImageItem}
          ...
          <div className="score-card-bg">
            <ScoreCard score={score} playAgainBtn={this.playAgainBtn} />
          </div>
        )}
      </div>
    )
